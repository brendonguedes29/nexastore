...
def _crud(request, model, formcls, titulo, template='gestao/lista_form.html'):
    ...
    for item in qs[:60]:
        ...
        registros.append({'id':item.pk,'titulo':str(item),'campos':campos})
...
def ferramentas(request):
    ...
    if request.method=='POST':
        ...
        if ferramenta and titulo:
            projeto=ProjetoQualidade.objects.create(loja=loja,ferramenta=ferramenta,titulo=titulo,problema=problema,responsavel=request.user.get_full_name() or request.user.username,dados=campos,status='andamento')
            perfil=getattr(request.user,'perfil_colaborador',None)
            if perfil:
                PontuacaoAtividade.objects.create(loja=loja,colaborador=perfil,categoria='qualidade',ferramenta=ferramenta,titulo=f'Aplicação {projeto.get_ferramenta_display()} #{projeto.id}',pontos=15)
                perfil.pontos+=15; perfil.nivel=1+(perfil.pontos//500); perfil.save(update_fields=['pontos','nivel'])
            messages.success(request,'Ferramenta salva. A atividade válida foi registrada no histórico e na evolução do participante.')
