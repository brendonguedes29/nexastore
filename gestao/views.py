...
def treinamento_atribuir(request,pk):
    ...
    if request.method=='POST':
        ...
        for c in Colaborador.objects.filter(loja=loja,pk__in=ids):
            ...
            trilha.prazo=prazo; trilha.save(update_fields=['prazo'])
...
def trilha_executar(request,pk):
    ...
    trilha=get_object_or_404(TrilhaColaborador,pk=pk,colaborador=perfil,loja=perfil.loja); etapas=list(trilha.treinamento.etapas.all()); atual=None
    for e in etapas:
